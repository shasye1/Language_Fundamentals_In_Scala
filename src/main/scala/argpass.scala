...
object argpass {
  ...
  def refint1 (f : RefInt => Unit) : (Int, Int, Int) = {
    ...
    // f (r)
    // val n : Int = r.get
    var x = new RefInt(0) //Your code must create ONLY ONE instance of RefInt
    var a = f(x)           // then call f three times.
    var aget = x.get
    var z = f(x)
    var zget = x.get
    var y = f(x)
  ...
  def refint3 (r : RefInt) : RefInt = {
    // TODO: Provide definition here.
    var x = new RefInt(r.get*2)   //return double the original value
    r.set(r.get+1)
    x                             //^
  }
  ...
  def refint4 (r : RefInt, f : RefInt => Unit) : Boolean = {
    // TODO: Provide definition here.
    var x = new RefInt(r.get)   //copy of RefInt r
    f(x)                         //f call on copy
    if (x.get == r.get) {true}    //Your code should return true if f has NOT changed the Int stored in the copy of r.
    else {
    false}                    //Otherwise it should return false.
  }
  ...
  def refint5 (xs : List[RefInt]) : Unit = {
    ...
    xs match{
      case Nil => Nil
      case x::xs => {
        x.set(x.get/2);         // Your code must CHANGE the argument it receives in a way that is visible to the caller.
        refint5(xs)
      }
      }
  }
