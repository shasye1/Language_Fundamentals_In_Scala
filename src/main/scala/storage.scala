...
object storage {
  ...
  val getAndSet : Int => (() => Int, Int => Unit) = {
    ...
    (n: Int) => {
      var x = n  //init with n
      def get: () => Int = () => x    // (n: Int) => (() => n);
      def set: Int => Unit = (a: Int) => x = a
      (get, set)
    }
  }
  ...
  val getAndSetSpy : () => (() => Int, Int => (() => Int, Int => Unit)) = {
    ...
    var total = 0
    () => {
      var counter: Int => (() => Int, Int => Unit) = {
        (n: Int) => {
          var x = n  //init with n
          def get: () => Int = () => x    // (n: Int) => (() => n);
          def set: Int => Unit = (a: Int) => {total = total + 1; x = a}
          (get, set)
        }
      }
      def getCount: () => Int = () => total
      def getSet2: Int => (() => Int, Int => Unit) = counter
      (getCount, getSet2)
    }
  }
}
