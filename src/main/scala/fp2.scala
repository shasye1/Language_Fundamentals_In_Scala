...
object fp2 {
  ...
  def append [A] (xs:List[A], ys:List[A]) : List[A] = {
    ...
    xs match {
     case List()=> ys
     case a :: tail  => a :: append(xs.tail,ys)
    }
  }
  ...
  def foldLeft [A,B] (xs:List[A], e:B, f:(B,A)=>B) : B = {
    ...
    xs match {
     case Nil => e
     case y::ys => foldLeft(ys,f(e,y),f)         //foldLeft()
    }
  }
  ...
  def joinTerminateRight (xs : List[String], term : String) : String = {
    // TODO: Provide definition here.
    def stringtermer(a:String, s:String) : String = term + a + s
    xs match {
      case Nil    => ""
      case y::ys  => y + foldRight(ys, term, stringtermer)
    }
  }
  ...
  def joinTerminateLeft (xs : List[String], term : String) : String = {
    // TODO: Provide definition here.
    def stringtermer(a:String, s:String): String = a + s + term
    xs match {
      case Nil => ""
      case y::ys => foldLeft(ys, stringtermer("", y), stringtermer)
    }
  }
  ...
  def firstIndexNumGreaterThan (a : Int, xs : List[Int]) : Int = {
    // TODO: Provide definition here.
    def AuxF(a : Int, xs : List[Int], i : Int) : Int = {
      xs match {
        case Nil    => throw new java.util.NoSuchElementException
        case y::ys  => if (y >= a)
                          i
