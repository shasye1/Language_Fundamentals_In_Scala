...
object subtyping {
  ...
  // I.e., if the increment method is called 3 times on an instance, and the decrement method is called 2 times on the
  // same instance, then it should store 5  (somewhere other than the existing field n).
  ...
  class otherCounter() extends Counter {          //Counter copy
    private var total = 0
    override def increment () = {super.increment();
                                 total = total + 1}
    override def decrement() = {super.increment();
                                 total = total + 1}
  ...
  def observeCounter (f : Counter => Unit) : Int = {
    ...
    val count = new otherCounter
    f(count)
    count.getTotal()    //IT WORKED
  }
  ...
  def observeCounterList (f : List[Counter] => Unit) : List[Int] = {
    // TODO: Provide definition here.
    val x = new otherCounter
    val y = new otherCounter
    val z = new otherCounter
    f(x::y::z::Nil)   //3
    (x.getTotal::y.getTotal::z.getTotal::Nil)
  }
  ...
  // The first element of the result Array[Int] must correspond to the number of times that
  // increment/decrement were called on the first element of type Array[Counter], similarly for the second and third elements.
  def observeCounterArray (f : Array[Counter] => Unit) : Array[Int] = {
    // TODO: Provide definition here.
    val a = new otherCounter
    val b = new otherCounter
    val c = new otherCounter
    f(List (a, b, c).toArray) //List to array
    (List(a.getTotal(), b.getTotal(), c.getTotal()).toArray)
  }
}
