...
object fp3 {
  ...
  def allEqual (xs : List[Int]) : Boolean = {
    ...
    xs match {
      ...
      case y::Nil => true       // - allEqual (List (5)) == true
  ...
  def stringLengths (xs:List[String]) : List[(String, Int)] = {
    // TODO: Provide definition here.
    xs.map(x => (x,x.length))     // return pair string,lenght
  }
  ...
  def delete1 [X] (x:X, ys:List[X]) : List[X] = {
    // TODO: Provide definition here.
    ys match {
      case Nil => Nil                       // delete1(3,List(1,2,3))
      case a::as => if (a == x)             // if 1 == 3, delete1(3,2)...
                        delete1(x,as)
                    else                    // else 1::delete1(3,2)....
                        a::delete1(x,as)
    }
  }
  ...
  def removeDupes2 [X] (xs:List[X]) : List[(Int, X)] = {
    // TODO: Provide definition here.
    def helper [X] (x:X, ys:List[X], a:Int) :List[(Int, X)] = {
        ys match {
          case Nil => if (x != Nil)
                          (a,x)::List()
                      else
                          List()
          case y::ys => if (y != x)
                          (a,x)::helper(y,ys,1)   //check next
                      else
                          helper(y,ys,a+1)          //+1 duplicate
          }
      }
    if (xs != Nil)
        helper(xs.head, xs.tail, 1)
    else
        List()
  }
  ...
  def splitAt [X] (n:Int, xs:List[X]) : (List[X], List[X]) = {
    ...
    (n, xs) match {
     case (_, Nil) => (Nil, Nil)
     case (-1, ys) => (List(), ys)
     case (n, x::xs) => val (a, b) = splitAt(n - 1, xs)
                        if (n > 0)
                            (x :: a,b)
                        else
                            (a, x::b)
